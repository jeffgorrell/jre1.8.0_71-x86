...
Try {
	...
	##	x64 installs fine with exe using /s and providing java.settings.cfg file
	##	x86 has problems with system32 and syswow64 redirection so extract the msi and pass it the same parameters from the java.settings.cfg file
	
	##*===============================================
	##* VARIABLE DECLARATION
	##*===============================================
	## Variables: Application
	[string]$appVendor = 'Oracle' #optional
	[string]$appName = 'Java Runtime Environment' #required (only leave blank to trigger $useDefaultMSI = $true)
	[string]$appVersion = '8u71' #optional
	[string]$appArch = 'x86' #optional (x86 or x64)
	[string]$appLang = '' #optional
	[string]$appRevision = '' #optional
	[string]$appScriptVersion = '1.0.0' #required
	[string]$appScriptDate = '29/01/2016' #required
	[string]$appScriptAuthor = 'Jeff Gorrell - UQBS IS Team' #optional
	##*===============================================
	...
	[string]$appVersionFull = '1.8.0_71' #optional (useful for $appInstaller when naming does not use $appVersion)
	[string]$appInstaller = "jre" + $appVersionFull + ".msi" #required (can be either .exe, .msi or .msp)
	[string]$appInstallerParams = '/passive /norestart AUTO_UPDATE=Disable EULA=Disable NOSTARTMENU=Enable WEB_ANALYTICS=Disable WEB_JAVA=Enable WEB_JAVA_SECURITY_LEVEL=H REMOVEOUTOFDATEJRES=1' #optional (if /S for exe be careful of required capitalisation)
	[string]$closeApps = 'javaw' #optional (Process "Name" as it appears in Get-Process)
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		# Remove only 32-bit versions of Java 8
		Remove-MSIApplications -Name "Java 8 Update" -FilteredApplication @(,,@('Is64BitApplication', $false, 'Exact'))
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		If ($useDefaultMsi) {
			...
			#>
			#Removes all versions of software that match the name "Java 8 Update" where the software is 32-bit and the publisher is "Oracle Corporation".
